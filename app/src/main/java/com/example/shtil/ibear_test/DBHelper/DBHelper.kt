...
class DBHelper (context: Context): SQLiteOpenHelper(context,DATABASE_NAME, null, DATABASE_VERSION ) {
    ...
    val allPerson: List <Person>
        get() {
            ...
            if(cursor.moveToFirst()){
                do {
                    val person = Person(cursor.getInt(cursor.getColumnIndex(COL_ID)), Uri.parse(cursor.getString(cursor.getColumnIndex(COL_IMG))),cursor.getString(cursor.getColumnIndex(COL_NAME)),cursor.getLong(cursor.getColumnIndex(
                        COL_PHONE)), cursor.getString(cursor.getColumnIndex(COL_EMAIL)))
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        db!!.execSQL("CREATE TABLE $TABLE_NAME($COL_ID INTEGER PRIMARY KEY AUTOINCREMENT,$COL_IMG TEXT, $COL_NAME TEXT," +
                " $COL_PHONE INTEGER, $COL_EMAIL TEXT)")
    }
